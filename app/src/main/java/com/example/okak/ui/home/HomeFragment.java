...
public class HomeFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        btnGenerateMenu.setOnClickListener(v -> {
            List<String> cuisines = new java.util.ArrayList<>();
            List<String> mealTypes = java.util.Arrays.asList("breakfast", "lunch", "dinner");
    ...
    private void setupObservers() {
        menuViewModel.getCurrentMenu().observe(getViewLifecycleOwner(), menu -> {
            if (menu != null && menu.days != null && !menu.days.isEmpty()) {
                ...
                MenuDayAdapter adapter = new MenuDayAdapter(new java.util.ArrayList<>());
                rvMenuDays.setAdapter(adapter);
